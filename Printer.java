...
public class Printer implements Printable, Pageable
{
  ...
  public boolean printUsingDialog ()
  {
    ...
    if (p.printDialog ())
    {
      ...
      maxLines = (int) (pageFormat.getImageableHeight () / new Canvas ().getFontMetrics (font).getHeight ());
      p.setPageable (this);
      try
      {
        p.print ();    
  ...
  public int print(Graphics g, PageFormat pf, int page) throws PrinterException 
  {
    ...
    for (int i = 0; i < images.size (); i ++)
    {
      ...
      else
      {
        ...
        int width = (int) (temp [0] / 100.0 * pf.getImageableWidth ());
        int height;
        if (temp [1] == -1)
          height = (int) (temp [0] / 100.0 * pf.getImageableWidth ());
        else
          height = (int) (temp [1] / 100.0 * pf.getImageableHeight ());
